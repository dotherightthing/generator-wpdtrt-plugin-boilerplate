 ...
 * Plugin class
 * Supports:
 *  Options page with field templating, error messaging, CSS
 *  Shortcode with CSS and JS with data object
 *
 * @package     WPDTRT_Plugin
 * @version     1.0.0
 *
 * @todo Replace placeholders with $this->properties
 * @todo Split shortcode into reusable class
 * @todo Make template-parts names match shortcode/widget names
 */
...
if ( !class_exists( 'WPDTRT_Plugin' ) ) {
  ...
  class WPDTRT_Plugin {
    ...
    function __construct( $prefix, $slug, $menu_title, $developer_prefix, $shortcode_name, $plugin_directory, $option_defaults, $shortcode_option_defaults ) {
      ...
      $this->set_shortcode_option_defaults( $shortcode_option_defaults );
     ...
     * @return array
     */
    public function get_shortcode_option_defaults() {
      return $this->shortcode_option_defaults;
    }
     ...
     * @param array
     */
    protected function set_shortcode_option_defaults( $new_shortcode_option_defaults ) {
      $this->shortcode_option_defaults = $new_shortcode_option_defaults;
    }
     ...
     */
    // attempt 1:
    // private function.. = Attempting to parse a shortcode without a valid callback
    public function render_shortcode( $atts, $content = null ) {
      ...
      $shortcode_options = shortcode_atts(
        $this->get_shortcode_option_defaults(),
        $atts,
        $this->get_shortcode_name()
      );

      // pass options to get_template_part()
      $options_all = array_merge( $shortcode_options, $options );
    ...
    /**
     * Render the appropriate UI on Settings page
     *
     ...
     * @return      string
     */
    public function render_options_page_field( $type, $name, $label, $tip=null ) {
       ...
       * e.g. $name="toggle_label" => $toggle_label => ('Open menu', 'text-domain')
      ...
      require($this->get_plugin_directory() . 'templates/options-' . $type . '.php');
    ...
    ?>
          <div class="notice notice-success is-dismissible">
            <p><?php echo
              ...
              $this->get_menu_title() . ' ' .
              __('settings successfully updated', '<%= name %>');
            ?></p>
          </div>
    <?php
        endif;
